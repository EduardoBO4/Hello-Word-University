...
public class Arvores {
    ...
    class ArvoreBicolor {
        ...
        private void balancear(No bisavo, No avo, No pai, No i) {
            if (pai == null || pai.cor == false) return; // Só balanceia se o pai for preto

            No novaRaiz = null;

            if (pai.valor > avo.valor && i.valor > pai.valor) novaRaiz = rotacionarEsq(avo);
            else if (pai.valor > avo.valor && i.valor < pai.valor) novaRaiz = rotacionarDirEsq(avo);
            else if (pai.valor < avo.valor && i.valor < pai.valor) novaRaiz = rotacionarDir(avo);
            else if (pai.valor < avo.valor && i.valor > pai.valor) novaRaiz = rotacionarEsqDir(avo);

            if (novaRaiz != null) {
                novaRaiz.cor = true;
                if (novaRaiz.esquerda != null) novaRaiz.esquerda.cor = false;
                if (novaRaiz.direita != null) novaRaiz.direita.cor = false;

                if (bisavo == null) this.raiz = novaRaiz;
                else if (novaRaiz.valor < bisavo.valor) bisavo.esquerda = novaRaiz;
                else bisavo.direita = novaRaiz;
            }
        }
