...
void selecao_nome(Restaurante* array, int n) {
    for (int i = 0; i < n - 1; i++) {
        int menor = i;
        for (int j = i + 1; j < n; j++) {
...
bool pesquisa_binaria(Restaurante* array, int n, char* nome_procurado) {
    int esq = 0;
    int dir = n - 1;
    while (esq <= dir) {
        int meio = (esq + dir) / 2;
        comp++;
        int r = strcmp(nome_procurado, array[meio].nome);
        if (r == 0) {
            return true;
        } else if (r < 0) {
            dir = meio - 1;
...
int main() {
    ...
    selecao_nome(selecionados, n_sel);
    ...
    while (fgets(linha, sizeof(linha), stdin) != NULL) {
        ...
        if (pesquisa_binaria(selecionados, n_sel, linha)) {
            printf("SIM\n");
        } else {
            printf("NAO\n");
        }
    ...
    FILE* arq_log = fopen(MATRICULA "_binaria.txt", "w");
    if (arq_log != NULL) {
        fprintf(arq_log, "%s\t%d\t%.4lf\n",
                MATRICULA, comp, total_tempo);
        fclose(arq_log);
    }
