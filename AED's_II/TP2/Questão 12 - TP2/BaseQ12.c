...
void pilha_inserir(Pilha* p, Restaurante r) {
    if (pilha_cheia(p)) {
        fprintf(stderr, "Erro ao inserir (pilha cheia)!\n");
        return;
    }
    p->array[p->topo] = r;
    p->topo++;
...
Restaurante pilha_remover(Pilha* p) {
    if (pilha_vazia(p)) {
        ...
        Restaurante vazio;
        vazio.id_restaurante = -1;
        vazio.nome = NULL;
        vazio.cidade = NULL;
        vazio.tipo_cozinha = NULL;
        return vazio;
    }
    p->topo--;
...
void pilha_mostrar(Pilha* p) {
    ...
    for (int i = p->topo - 1; i >= 0; i--) {
        formatar_restaurante(&p->array[i], buffer);
        printf("%s\n", buffer);
    }
}
...
int main() {
    ...
    while (fgets(linha, sizeof(linha), stdin) != NULL) {
        ...
        if (pos != -1) {
            pilha_inserir(&pilha, cr->restaurante[pos]);
        }
    }

    if (fgets(linha, sizeof(linha), stdin) == NULL) return 0;
    int n = transformar_int(linha);
    ...
    for (int k = 0; k < n; k++) {
        ...
        if (linha[0] == 'I') {
            ...
            if (pilha_vazia(&pilha)) continue;
            Restaurante removido = pilha_remover(&pilha);
            printf("(R)%s\n", removido.nome);
        }
    }
