...
public class Main {
	...
	public static int comparar(Restaurante a, Restaurante b) {
		Data da = a.getDataAbertura();
		Data db = b.getDataAbertura();
    ...
    for (int i = 1; i < n; i++) {
        Restaurante ref = array[i];
        mov++; 
        int j = i - 1;

        while (j >= 0) {
            comp++; 
            if (array[j].getCidade().compareTo(ref.getCidade()) > 0) {
                array[j + 1] = array[j];
                mov++; 
                j--;
            } else {
	...
	public static void main(String[] args) {
		...
		try {
			FileWriter arqLog = new FileWriter("890309_insercao.txt");
