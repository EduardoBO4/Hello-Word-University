...
public class Main {
    ...
    public static boolean pesquisaSequencial(Restaurante[] array, int n, String nomeProcurado) {
        for (int i = 0; i < n; i++) {
            comp++; 
            if (array[i].getNome().equals(nomeProcurado)) {
                return true; 
            }
        }
        return false; 
    ...
    public static void main(String[] args) {
        ...
        while (leitor.hasNextLine()) {
            ...
            if (nomeBuscado.trim().equals("FIM")) break;

            if (pesquisaSequencial(selecionados, qtd, nomeBuscado)) {
        ...
        try {
            FileWriter arqLog = new FileWriter("890309_sequencial.txt");

            arqLog.write(String.format("890309\t%d\t%.4f%n", comp, tempoMs));
            arqLog.close();
        } catch (IOException e) {
