...
int mov = 0, comp = 0;//variaveis para contagem de comparacoes e movimentos
   ...
   /*pequeno teste para ver se esta funcionando
    ...
    while(strcmp(linha, "-1") != 0){//comparo se é diferente de -1
        ...
        if(idBuscado != -1){//verifico se é diferete de -1
            inserir(&(cr->restaurante[idBuscado]), h);//fomato o restaurante e passo para o char leitura
        }
