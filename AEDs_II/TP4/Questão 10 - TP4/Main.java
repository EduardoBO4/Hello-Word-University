...
class NoBSTChar {
    public char chave;
    public NoTrie filho;
    public NoBSTChar esq, dir;
    public NoBSTChar(char chave, NoTrie filho) {
        this.chave = chave; this.filho = filho; this.esq = this.dir = null;
    }
}

class NoTrie {
    public Restaurante restaurante;
    public NoBSTChar raizFilhos;
    public NoTrie() { this.restaurante = null; this.raizFilhos = null; }
}
...
class TrieBST {
    ...
    public TrieBST() { this.raiz = new NoTrie(); this.comparacoes = 0; }

    private NoBSTChar inserirBST(NoBSTChar no, char c) {
        if (no == null) return new NoBSTChar(c, new NoTrie());
        if (c < no.chave) no.esq = inserirBST(no.esq, c);
        else if (c > no.chave) no.dir = inserirBST(no.dir, c);
        return no;
    }
    ...
    public void inserir(Restaurante r) {
        ...
        for (int i = 0; i < nome.length(); i++) {
            ...
            NoTrie filho = buscarBST(atual.raizFilhos, c);
            if (filho == null) {
                atual.raizFilhos = inserirBST(atual.raizFilhos, c);
                filho = buscarBST(atual.raizFilhos, c);
            }
            atual = filho;
        }
        atual.restaurante = r;
...
public class Main {
    ...
    public static void main(String[] args) throws Exception {
        ...
        FileWriter arq = new FileWriter(MATRICULA + "_arvore_trie_arvore.txt");
